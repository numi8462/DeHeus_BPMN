...
const listProcesses = async (req, res) => {
  ...
  try {
    let role = '';
    if (userName && userName.includes('.pbmn@')) {
      // admin accounts manage every project directly
      role = 'admin';
    } else {
      ...
      // console.log('Contribution Query Result:', contributionResult.rows);  // 디버깅 용도라서 주석 처리!!
    ...
    const query = `
      ...
      FROM
        ...
        LEFT JOIN diagram_checkout dc ON d.id = dc.diagram_id AND dc.status = true
    ...
    const projectName = result.rows.length > 0 ? result.rows[0].projectName : 'Unknown Project';
...
const addProcess = async (req, res) => {
  ...
  try {
    ...
    const diagramId = insertResult.rows[0].id;
    await pool.query(
